...
| **I declare what I could not see.** | Every report states its own **coverage**: which files a limit skipped, which modules crashed, which dimensions had no surface to audit at all. A dimension with nothing to check scores **"out of scope"**, never 10/10 — because *"I couldn't look"* must never read as *"I looked, it's clean"*. | `report/coverage.ts` Coverage Manifest · `--max-depth` / `--max-files` to lift the limits · `test/coverage.test.ts` |
...
- **Module B — SAST** (passive): hardcoded secrets (+ provider keys & **entropy‑based** detection + **git‑history** scan), vulnerable dependencies (`npm/pnpm audit`), weak crypto (MD5/SHA1/ECB/CryptoJS + **static IV**), **`alg:none`** & **weak JWT secrets**, IDOR, SQL/**NoSQL**/**LDAP**/command/eval injection, **SSRF · SSTI · path traversal · insecure deserialization · XXE · open redirect**, CORS — across **TS/JS, Python, PHP, C#, Go**.
...
- **Module FE — Frontend security** (passive): the browser's own attack surface, split out from Module B in v0.10 so a **clean frontend actually scores 10/10** instead of showing `n/d`. DOM‑XSS sinks (`dangerouslySetInnerHTML`, Vue `v-html`, Svelte `{@html}`, Astro `set:html`, `innerHTML`/`document.write`, `javascript:` URLs), **tokens in web storage**, **weak CSP** — including the multi‑line `helmet`/Next.js form a line‑based scanner structurally cannot see — **prod source‑maps**, `postMessage` wildcard origin & unvalidated `message` listeners, **reverse tabnabbing**, and **missing SRI** on external CDN scripts. Scans `.html/.njk/.ejs/.hbs` templates too; only runs when a frontend surface is detected.
- **Module C — DAST** (active, **authorization‑gated**): a **43‑path exposed‑file catalog** — env variants, private keys (SSH/TLS/GCP service account), VCS trees (`.git/index`, `.svn/wc.db`, `.hg`), app config (`web.config`, `appsettings.json`, `settings.py`, `wp-config.php.bak`), DB dumps, and debug surfaces (`actuator/env`, `debug/pprof`, `metrics`, Symfony profiler, `phpinfo`, `server-status`) — plus **directory listing (autoindex)**, security headers + TLS, open admin panels, rate‑limit probing, cookie flags, sensitive port inventory. Every path carries a **content validator**, so a SPA that returns 200 for everything produces zero findings; heap/thread dumps are deliberately excluded to keep probing non‑intrusive.
- **Coverage Manifest** (always on): the report's own honesty layer — files skipped by a depth/size/count limit, rules that hit their per‑file cap (*"there may be more than shown"*), modules that crashed, and modules that ran but found no surface. Scoreboard gains a third state, **"out of scope"**, which is excluded from the overall average.
